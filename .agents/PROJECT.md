...
|---|------|-------|-------------|--------|
| 1 | M1: Next.js 16 & i18n Fixes | Fix middleware matcher / proxy.js, add missing `/es/login` page, root page redirect, async `params` in RootLayout, and `generateStaticParams()` | none | PLANNED |
| 2 | M2: E2E Test Suite Creation | Install Playwright, configure `playwright.config.ts`, write Tiers 1-4 E2E test specs | M1 | PLANNED |
| 3 | M3: Final E2E Validation & Hardening | Run local production server, execute 100% passing Playwright tests, adversarial hardening | M1, M2 | PLANNED |
